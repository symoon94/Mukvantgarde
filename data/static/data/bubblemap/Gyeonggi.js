...
var projection = d3.geo.mercator()
    ...
    .scale(20000)
    .translate([width/1.6, height/1.5]);

var path = d3.geo.path().projection(projection);

d3.json("/static/topojson/Gyeonggi-do.json", function(error, data) {
    var features = topojson.feature(data, data.objects["Gyeonggi-do"]).features;
...
d3.csv("/static/data/instadata/states/Gyeonggi-do.csv", function(data) {
  console.log(data);
  ...
  function update() {
    ...
    if(cb.property("checked") && grp !== "all"){
      places.selectAll("."+grp).transition().duration(1000).style("opacity", 0.7).attr("r", function(d){ return d.post/6000 })
    }else{
